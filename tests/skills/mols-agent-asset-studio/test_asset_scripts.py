...
SKILL = ROOT / "src/rulesync/.rulesync/skills/mols-agent-asset-studio"
...
def test_inventory_finds_single_studio_entrypoint(tmp_path: Path) -> None:
    ...
    assert "src/rulesync/.rulesync/skills/mols-agent-asset-studio/SKILL.md" in paths
    for name in REMOVED_SPLIT_SKILLS:
        assert f"src/rulesync/.rulesync/skills/{name}/SKILL.md" not in paths
