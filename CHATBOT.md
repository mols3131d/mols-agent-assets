...
1. 구체적인 GitHub repository나 object를 읽거나 변경하거나 GitHub tool action을 수행하는 task라면, 기억이나 이전 대화만 믿지 말고 현재 repository와 관련 ref/object를 live state에서 먼저 식별합니다.
1. 작업 대상 경로가 정해지면 관련 ref의 repository root부터 해당 경로까지 존재하는 `AGENTS.md`를 확인하고, 더 좁은 scope의 instruction이 있으면 함께 적용합니다.
1. 이어서 [`.agents/route/ROUTE.md`](.agents/route/ROUTE.md)를 읽고 현재 작업에 필요한 Agent Asset만 선택해 로드합니다. 구체적인 GitHub task에서는 narrow family 하나만으로 `github-context`를 놓치지 않도록 `all` route에서 `github-context`와 task-specific Asset을 함께 선택하고, GitHub task가 아니면 `ROUTE.md`의 일반 family selection을 따릅니다.
