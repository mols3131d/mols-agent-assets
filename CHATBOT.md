...
1. 구체적인 GitHub repository/object(`repository`, `path`, `ref`, Pull Request, Issue, check, workflow, release 등)를 대상으로 읽기·변경·tool action을 수행하는 task라면, 기억이나 이전 대화만 믿지 말고 현재 repository와 관련 ref/object를 live state에서 먼저 식별합니다.
...
1. 구체적인 GitHub task에서는 이후 작업이 그 context에 의존하기 전에 `github-context`가 현재 repository/ref/object, 적용되는 repository instruction과 task에 필요한 live GitHub state를 확인하도록 합니다. 세부 loading 범위와 stop condition은 해당 Skill이 소유합니다.
