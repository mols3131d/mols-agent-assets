...
- `src/agentsmesh/agentsmesh.yaml`: isolated native AgentsMesh workspace의 projection configuration.
- `src/agentsmesh/.agentsmesh/rules/`: AgentsMesh-compatible Rule canonical source.
- `src/agentsmesh/.agentsmesh/skills/`: AgentsMesh-compatible Skill canonical source.
- `src/agentsmesh/.agentsmesh/agents/`: AgentsMesh-compatible Agent canonical source.
...
Repository root의 `.agentsmesh/`, `.github/skills/`, `.github/agents/`, `.github/copilot-instructions.md`, `.agents/rules/`, `.agents/skills/`는 canonical 또는 generated distribution surface로 commit하지 않습니다. `src/agentsmesh/.agentsmesh/`만 distribution source로 사용합니다.
...
1. AgentsMesh가 표현할 수 있는 Rule, Skill, Agent는 `src/agentsmesh/.agentsmesh/`에서 작성하거나 수정합니다.
1. 필요한 경우에만 자산별 maintainer docs를 함께 갱신합니다.
1. Markdown 변경은 repository rumdl policy에 맞춰 format합니다.
1. Read-only AgentsMesh validation은 `src/agentsmesh/`에서 직접 수행합니다.
1. Generation처럼 파일을 쓰는 native validation은 `src/agentsmesh/` workspace 전체를 temporary directory로 복사한 뒤 수행합니다. Persistent `.lock`이나 generated target을 이 저장소의 drift baseline으로 두지 않습니다.
1. 필요한 repository test/eval을 실행합니다.
1. canonical source를 검토합니다. generated projection과 AgentsMesh lock state는 commit하지 않습니다.
