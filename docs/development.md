...
AgentsMesh가 표현하지 못하는 target semantics를 portability 명목으로 삭제하거나 축소하지 않는다. 명시적인 target-specific exception으로 남기고 그 이유를 필요한 maintainer 문서나 applicable reference에 기록한다.
