...
4. **Two distinct vantages** — Verifier와 Challenger는 같은 checklist의 표현 변형이 아니라 서로 다른 decision-relevant failure lens를 가진다.
...
2. 독립적인 두 failure lens
3. evidence-gated truth
4. authority-aligned action
5. bounded execution과 termination
6. 실제 가능한 병렬 효율
7. signal density
...
Evidence와 claim 의미는 [review-model.md](review-model.md), delegation·handoff·capability 유지보수는 [maintenance.md](maintenance.md)가 소유한다.
