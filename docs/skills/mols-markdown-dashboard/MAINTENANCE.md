...
1. Asset package의 Example YAML과 rendered Markdown을 함께 수정합니다.
1. Project와 domain 예제를 모두 렌더링합니다.
1. 3개 핵심 표의 열과 의미가 유지되는지 확인합니다.
1. Package-local 품질 게이트를 실행합니다.
1. Host repository가 이 asset을 위한 tests/evals를 제공하면 해당 repository의 verification contract에 따라 별도로 실행합니다.
1. 새 durable decision이나 recovery knowledge가 생긴 경우에만 maintainer docs에 반영합니다. 일반 작업 로그는 host repository의 history에 맡깁니다.

이 capsule과 `baseline/`은 source maintenance/recovery용 non-runtime surface입니다. Runtime 동작이 이 문서를 읽어야만 정상 작동하도록 만들지 않습니다.

## Package-Local Quality Gate

대응 `mols-markdown-dashboard` asset package root, 즉 `SKILL.md`, `pyproject.toml`, `scripts/`가 있는 directory에서 실행합니다.

```bash
uv run --with ruff --with ty --with rumdl python scripts/check_quality.py
```
...
하나라도 실행할 수 없거나 실패하면 package-local gate를 통과로 기록하지 않습니다. Host repository의 tests/evals는 이 script의 책임이 아니며, 존재하면 host verification을 추가로 수행합니다.

## Render Examples

Asset package root에서 실행합니다.
...
- 실행하지 못한 품질 검증은 현재 change에서 명시하되 session log를 durable maintainer doc로 누적하지 않습니다.
