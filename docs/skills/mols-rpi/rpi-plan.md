...
Plan은 현재 Scope 안에서 **어떤 state change를 어떤 ordered Work로 만들며 무엇으로 accept할지**를 충분히 설명해야 합니다. 그러나 세부 절차를 과잉 고정하는 것이 목적은 아닙니다.

- Work가 실제로 의존하는 decision과 ordering을 포함합니다.
...
Plan은 현재 Goal과 Scope에 필요한 Work를 안전하게 시작할 만큼 coverage가 유효할 때 downstream Work로 넘어갑니다. Plan 자체가 requested terminal이면 Review가 Plan과 그 Research lineage를 검증한 뒤 accept 여부를 결정합니다.
...
이를 one-shot 계획, 전체 재작성, speculative expansion 또는 ceremony를 위한 replanning으로 바꾸지 않습니다.
