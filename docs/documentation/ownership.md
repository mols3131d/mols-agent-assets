...
README의 생성·작성과 scope metadata는 [README Authoring](readme-authoring.md)이 소유합니다. 작은 README inline `Index`와 generated `INDEX.tsv` 중 어떤 indexing surface를 사용할지는 [Document Indexing](indexing.md)이 소유합니다.

단순 sibling 목록을 authored surface에 복제하지 않습니다. 사람이 읽는 selection cue나 generated discovery가 실제로 필요할 때만 가장 작은 적절한 indexing surface를 둡니다.
