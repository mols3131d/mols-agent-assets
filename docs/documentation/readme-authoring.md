...
1. 사람이 고를 수 있는 작은 탐색 안내가 필요하면 README 안에 `Index` 섹션을 둘 수 있습니다.

이 순서는 고정된 섹션 구성이 아닙니다. 필요 없는 단계나 섹션은 만들지 않습니다.

## Writing Rules

- 핵심 정보와 중요한 조건을 문단과 섹션 앞에 둡니다.
- 한 문단에는 하나의 핵심만 두고, 내용을 바로 알 수 있는 제목과 얕은 계층으로 빠르게 훑을 수 있게 합니다.
...
- 배지나 상태 요약은 현재 상태를 정확히 반영하고 독자의 판단에 도움이 될 때만 둡니다. 장식용 배지 나열은 만들지 않습니다.
- `Welcome`, `Overview` 같은 형식적 서론이나 README 자체를 설명하는 문장보다 이 범위의 핵심을 바로 제시합니다.

## Inline Index

사람이 문서를 고르는 데 작은 탐색 안내가 실제로 필요하면 README 안에 `Index` 섹션을 둘 수 있습니다. README의 `Index`와 자동 생성 `INDEX.tsv` 중 무엇을 사용할지는 [Document Indexing](indexing.md)의 원칙을 따릅니다.
...
디렉터리 메타데이터의 원본은 해당 디렉터리의 `README.md` 하나로 한정합니다. `README.md`에 frontmatter가 없거나 필요한 필드가 비어 있어도 다른 파일에서 값을 가져오거나 여러 메타데이터를 합치지 않습니다.

문서나 파일 묶음의 README도 같은 범위 메타데이터 원칙을 따릅니다. 특정 도구가 이 메타데이터를 어떻게 사용하는지는 해당 도구의 문서가 정합니다. 일반 문서 frontmatter의 필드와 스키마는 [Frontmatter](frontmatter.md)를 따릅니다.
...
- 각 섹션이 독자의 이해·선택·행동을 실제로 바꾸는가? 그렇지 않다면 제거하거나 더 적절한 정본으로 옮겼는가?
...
- frontmatter 필드와 탐색 메타데이터 → [Frontmatter](frontmatter.md)
