...
README 안의 inline `Index`, authored `INDEXING.md`와 generated `INDEX.tsv`의 선택·승격·중복 경계는 [Document Indexing](indexing.md)이 소유합니다.

`INDEXING.md`는 사람이 작성하는 directory indexing 전담 문서이므로 일반 documentation frontmatter contract를 따르지만 directory metadata source는 아닙니다. `INDEX.tsv`는 generated projection이므로 authored frontmatter source가 아닙니다.
