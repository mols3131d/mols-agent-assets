---
description: README inline Index, authored INDEXING.md와 generated INDEX.tsv 중 적절한 문서 인덱싱 surface를 선택하고 생성·승격·유지보수 경계를 결정할 때 사용하는 repository documentation policy입니다.
---

# Document Indexing

문서 인덱싱은 entrypoint를 대신하지 않습니다. 필요한 탐색 책임만 가장 가벼운 surface에 두고, 사람이 큐레이션하는 navigation과 생성 가능한 inventory를 분리합니다.
...
| `README.md`의 `Index` section | 작은 scope의 inline curated navigation |
| directory의 `INDEXING.md` | 독립적인 사람이 읽는 indexing 전담 문서 |
| directory의 `INDEX.tsv` | source에서 다시 만들 수 있는 generated indexing projection |

기본값은 **별도 index를 만들지 않는 것**입니다. 독자의 선택이나 도구의 discovery를 실제로 개선할 때만 가장 작은 적절한 surface를 추가합니다.
...
- grouping, reading order 또는 관계 설명이 독립적인 문서 책임이 될 만큼 복잡하지 않은 경우
...
Inline index가 길어지거나 category·reading order·관계 설명 자체가 독립적인 maintainer responsibility가 되면 `INDEXING.md`로 승격합니다. README 작성 자체의 원칙은 [README Authoring](readme-authoring.md)이 소유합니다.
...
사람에게 설명해야 하는 grouping이나 reading order를 `INDEX.tsv`에 숨기지 않습니다. 반대로 deterministic하게 다시 만들 수 있는 inventory를 `INDEXING.md`에 손으로 복제하지 않습니다.

## Coexistence

같은 scope에 둘 이상의 indexing surface가 존재할 수는 있지만, 각각 다른 선택 문제를 해결해야 합니다.

- README inline `Index`와 `INDEX.tsv`가 함께 있으면 README table은 generated inventory에 없는 사람용 selection cue를 소유해야 합니다.
- `INDEXING.md`와 `INDEX.tsv`가 함께 있으면 전자는 curated navigation, 후자는 generated discovery projection을 소유합니다.
- README와 `INDEXING.md`에 같은 curated table을 복제하지 않습니다.

한 surface를 제거해도 다른 surface가 자기 역할을 잃지 않는지 확인하면 중복 경계를 판단하기 쉽습니다.
...
- directory entrypoint인 `README.md`와 authored indexing artifact인 `INDEXING.md`는 generated inventory row로 중복하지 않습니다.
- generated `INDEX.*`, `AGENTS.md`, systemic·hidden Markdown과 문서가 없는 subtree는 generator의 selection rule에 따라 제외합니다.

예약 filename과 일반 문서 filename을 구분합니다. `INDEXING.md`와 generated `INDEX.*`는 indexing artifact이지만, `docs/documentation/indexing.md` 같은 일반 lowercase 문서는 이름이 `index`로 시작한다는 이유만으로 제외하지 않습니다.

따라서 새 nested directory가 생겼다는 이유만으로 그 안에 `INDEX.tsv`를 수동으로 추가하지 않습니다. 작은 local scope에서 README inline `Index`면 충분하다면 별도 generated index를 요구하지 않습니다. 더 깊은 위치에 `INDEX.tsv` materialization이 실제로 필요하다면 개별 파일을 추가하는 대신 generator policy와 그 필요성을 함께 변경합니다.
...
1. README inline `Index`, `INDEXING.md`, 일반 Markdown 또는 frontmatter의 authored source를 수정합니다.
1. generated docs projection에 영향을 주는 변경이면 `mise run generated-sync`를 실행합니다.
1. `INDEX.tsv` diff가 source 변경의 예상 결과인지 검토합니다.
1. inline index와 `INDEXING.md`가 generated inventory를 불필요하게 복제하지 않는지 확인합니다.
