...
전체 source를 처음부터 읽지 않고도 어떤 후보가 있고 무엇을 더 확인하거나 선택해야 하는지 판단할 수 있게 하여 탐색 비용과 불필요한 context load를 줄입니다.

## Core

Index와 Routing은 겹칠 수 있지만 목적이 다릅니다.
...
- Path나 file list만 제공해도 유효한 Index가 될 수 있지만, semantic selection이 필요한 Routing이라면 소비자가 후보를 실제로 고를 만큼의 signal이 필요합니다.
...
Routing signal의 형태는 고정하지 않습니다. Description, trigger, `when to use`, tag, rule, category나 사람이 읽는 짧은 설명 등 현재 후보를 구분하고 선택할 수 있는 표현이면 충분할 수 있습니다.
...
Installation, generation, synchronization, update나 다른 mutation은 이 패턴의 책임이 아닙니다. Routing은 필요하면 해당 operation을 소유하는 tool이나 capability로 연결할 수 있습니다.
...
별도 routing/index asset이 관리 비용을 정당화하지 못하는 작은 범위에서는 entrypoint나 가까운 guide의 table, bullet, prose로 routing 정보를 직접 제공하는 편이 더 단순할 수 있습니다.
...
Generated routing/index는 underlying source나 tool-managed asset의 authority를 가져오지 않습니다. 생성 원천과 projection의 관계가 중요하면 그 경계를 유지합니다.
...
- Routing asset이 stale하면 잘못된 후보나 surface를 선택하게 할 수 있으므로 source나 관리 방식의 변경과 함께 갱신하거나 검증하는 방법을 고려합니다.
- 사람이 읽는 탐색과 agent가 소비하는 context routing은 같은 source를 공유하거나 서로 다른 projection을 사용할 수 있습니다.
- 외부 candidate를 발견했다고 해서 현재 runtime에 설치·활성화·접근 가능하다고 가정하지 않습니다.
- Generated 또는 installed asset이 존재한다는 사실만으로 그 위치가 canonical authority라고 가정하지 않습니다.
- 자동화 비용이 수동 관리보다 크면 단순한 수동 index나 inline routing이 더 적합할 수 있습니다.
...
Routing/index asset은 underlying 문서·코드·정책·동작이나 tool-managed asset의 canonical owner가 되지 않습니다. 다만 **routing 자체에 필요한 identity, 위치, 분류, 선택 기준, routing intent와 next route**는 이 layer가 소유할 수 있습니다.

이 패턴은 특정 schema, generator, directory, filename, format, source type, manager taxonomy 또는 routing algorithm을 강제하지 않습니다. 예시 이름과 surface는 discovery/routing intent를 설명하기 위한 representative form이며 실제 적용은 framework, runtime, source authority와 repository convention에 맞게 조정합니다.
