...
Filesystem이 **구조를 빠르게 이해하고 탐색하기 위한 정보 표면으로도 활용될 수 있는 상태**를 추구하는 패턴입니다.

Directory와 file의 이름, 배치, 계층, entrypoint를 보았을 때 무엇이 어디에 있고 어디부터 살펴볼지에 대한 큰 윤곽을 빠르게 잡을 수 있으면 좋습니다.

다만 **FS가 구조를 결정하지 않습니다.** FS legibility는 구조 선택의 보조 기준이며, 구조의 적합성·유효한 convention·유지보수성과 operability를 희생시키는 근거가 되어서는 안 됩니다.

이 패턴의 목표는 FS만으로 system을 정확히 복원하는 것이 아니라 **orientation과 navigation 비용을 낮추는 것**입니다.

## Boundary

이 원칙은 source code, 문서, configuration, scripts, tests 등 repository에서 file과 directory로 드러나는 구조 전반에 적용할 수 있습니다.

적용 여부와 강도는 대상, ecosystem과 **실제 navigation environment**에 따라 달라집니다. Tree, CLI, browser가 주요 탐색 수단이면 FS legibility의 가치가 커질 수 있고, IDE symbol navigation, search, dependency view가 이미 더 좋은 경로를 제공한다면 가중치는 낮아질 수 있습니다. Framework, generator, package manager처럼 layout owner가 따로 있는 영역은 그 convention을 우선합니다.
...
Legibility를 개선할 때는 **가장 싼 수단부터** 봅니다.
...
- tree만으로 부족하다면 작은 README나 index가 더 싼 해결책인지 봅니다.
- hierarchy, file split, placement처럼 filesystem organization 자체를 바꾼다면 줄어드는 탐색·이해 비용이 생기는 변경·운영 비용보다 충분히 큰지, 그리고 자연스러운 구조와 convention을 왜곡하지 않는지 봅니다.

핵심은 이 패턴 때문에 새로운 구조를 만드는 것이 아니라 **구조가 가능하면 탐색에도 도움을 주게 하는 것**입니다. 정보량이나 directory 수를 늘리는 것 자체는 legibility가 아닙니다. Tree에서 큰 윤곽과 다음 탐색 지점을 얻을 수 있으면 충분합니다.

## Guardrails

**FS legibility보다 correctness, 적절한 구조, 유효한 ecosystem convention, 유지보수성, operability가 우선합니다.**
