...
자산 유형 하나에만 적용되는 규격은 해당 유형 디렉터리에 둔다. 공통 문서가 유형별 세부 규격을 다시 정의하지 않는다.

## 구성

| 디렉터리 | 책임 |
| --- | --- |
| `standards/` | 자산 유형의 공통 기준, 저장소 로컬 분류 체계와 이름 규칙 |
...
- 자산 유형이나 배치 위치를 판단한다 → [Personal Agent Asset Standard](standards/agent-assets-standard-personal.md)
...
아니라면 유형별 레퍼런스가 더 적절하다. 미래에 공통화될 것이라는 예상만으로 공통 추상화를 만들지 않는다.
