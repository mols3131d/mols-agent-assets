...
이 문서는 Rulesync가 소유하지 않는 **repository integration boundary**만 정의합니다. Schema, file format, feature, target namespace와 projection behavior는 current upstream이 authoritative합니다.
