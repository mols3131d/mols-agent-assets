...
좋은 instruction은 모델이 **언제 무엇을 해야 하는지**를 불필요하게 추론하지 않게 합니다.

## Authoring Rules

1. **행동을 직접 씁니다.** 설명만 두지 말고 required action을 명시합니다.
1. **trigger와 action을 가까이 둡니다.** 조건, 행동, validation을 한 흐름으로 읽을 수 있게 합니다.
1. **default를 하나 둡니다.** 대안은 default를 벗어나는 조건이 있을 때만 노출합니다.
1. **같은 개념에는 같은 용어를 씁니다.** 문체를 위해 동의어를 섞지 않습니다.
1. **scope와 precedence를 명시합니다.** 동시에 적용될 수 있는 instruction의 충돌을 모델에게 추측시키지 않습니다.
1. **형식이 계약이면 예시나 template을 보여줍니다.** Prose보다 정확할 때만 사용합니다.
1. **실패 비용에 맞춰 자유도를 조절합니다.** 열린 문제는 goal/criteria 중심, 취약한 작업은 exact step/guardrail 중심으로 씁니다.
1. **validation을 관찰 가능하게 씁니다.** 실행하지 않은 검증을 추정하지 않게 합니다.
...
행동을 바꾸지 않는 설명은 제거 후보로 보고, 조건부 상세 context는 필요한 시점에만 로드합니다. 세부 판단은 [Design Principles](../principles/README.md)를 따릅니다.
