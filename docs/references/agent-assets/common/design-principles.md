...
Local Delta가 필요해도 prose instruction이 항상 올바른 mechanism은 아닙니다. 이미 존재하는 source/target-native mechanism, selector, permission, schema, validator 또는 deterministic automation이 해당 concern을 더 직접적으로 소유하고 검증할 수 있다면 그 owner를 우선합니다. Semantic judgment와 예외 판단처럼 모델이 해석해야 하는 behavior만 instruction으로 남깁니다.
...
- 더 직접적인 native/deterministic owner가 있는데 prose로 다시 구현하고 있지 않은가?
