...
Agent Skill은 **특정 영역의 지식과 반복 가능한 절차를 에이전트가 필요할 때 불러
쓰게 만드는 경량 패키지**다. 좋은 Skill은 많은 정보를 담은 문서가 아니라, 올바른
요청에서 활성화되어 필요한 지침만 제공하고 결과를 검증하는 실행 단위다.

이 문서는 설계와 작성 방법을 다룬다. Portable 형식과 front matter 규격은
[Agent Skills Specification](agent-skills-io/agent-skills-io-specification.md),
이 저장소의 추가 convention은
...
이 **Progressive Disclosure** 구조는 초기 context와 token을 줄이고 관련 없는
지침의 간섭을 막는다.

- Discovery: 모든 Skill의 `name`, `description`만 노출
- Activation: 요청과 일치하는 Skill의 `SKILL.md` 로드
- Execution: 필요한 resource만 사용

Skill은 반복 작업, 프로젝트 고유 규칙·API·schema, 엄격한 결과 형식, 비자명한
예외가 있을 때 만든다. 단순한 일회성 요청이나 모델이 이미 잘하는 일반 작업에는
만들지 않는다.
...
작성 전에 적용할 authority를 구분한다.

- Portable `SKILL.md`, front matter, 표준 resource convention →
  [Agent Skills Specification](agent-skills-io/agent-skills-io-specification.md)
- Repository-local Skill extension →
  [Personal Skill Standard](agent-assets-skills-standard-personal.md)
- Target profile, flat/runtime 경계, package surface →
  [Skill Target Profiles](agent-assets-skills-target-profiles.md)

Guide는 이 규격을 다시 정의하지 않는다. 프로젝트나 target runtime이 별도 mandatory
contract를 제공하면 그것을 함께 적용한다.

일반적인 directory-based Skill은 `SKILL.md`를 중심으로 필요한 resource만
추가한다. 빈 `references/`, `scripts/`, `assets/`를 형식상 만들지 않는다.

## 4. 만드는 순서

### 1) 실제 지식 수집

성공한 작업 순서, 사용자 교정, 실제 patch, runbook, schema, post-mortem에서
반복되는 판단과 실패 지점을 찾는다. 일반 지식을 다시 설명하지 않는다.
...
공통 trigger와 workflow를 공유하면 하나의 Skill 안에서 route한다. 목적이나
권한이 다르면 Skill을 나눈다.

### 3) `description` 작성

활성화 전에는 `description`이 discovery에 사용된다. 내부 구현이 아니라 사용자
intent를 쓴다.
...
정확한 keyword가 없는 간접 요청도 포괄하되, 단어가 겹치는 near-miss는 제외한다.
`Helps with data`처럼 넓고 모호한 표현은 피한다.

Field constraint는
[Specification](agent-skills-io/agent-skills-io-specification.md)을 따르고,
trigger 품질을 체계적으로 개선할 때는
...
파일을 나눠도 시작할 때 전부 읽으면 Progressive Disclosure가 아니다. 각 resource의
load condition을 `SKILL.md`에 명시한다.
...
False Negative는 scope를 넓힐 신호이고 False Positive는 경계를 좁힐 신호다.
실패 문구를 `description`에 그대로 복사하지 말고 실패한 intent를 반영한다.
...
Static format과 front matter는 Specification이 안내하는 validator와
repository-owned deterministic check로 별도 검증한다.
...
- 표준 규격을 Guide나 workflow에 복사해 여러 owner를 만든다.
...
- [ ] 하나의 일관된 사용자 목적을 다룬다.
- [ ] Specification과 적용 가능한 Personal Standard를 확인했다.
...
- [ ] 적용 가능한 deterministic validation을 실행했다.
