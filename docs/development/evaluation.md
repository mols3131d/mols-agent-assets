...
여기서 regression은 **평가 목적**을 뜻하며 별도의 파일 category를 의미하지 않습니다. Agent behavior를 실행하지 않아도 판정할 repository invariant는 `tests/`에 두고, behavioral regression fixture는 자산 유형에 맞는 `evals/skills/<skill-name>/` 또는 `evals/subagents/<subagent-or-family>/`에 둡니다.
