...
다만 탐색성을 위해 구조를 왜곡하지 않습니다. Correctness, 명확한 ownership, 자연스러운 ecosystem convention, 유지보수성과 operability가 우선하며, framework·tool·generated 영역에 더 구체적인 layout 규칙이 있으면 그 규칙을 따릅니다.
...
- 새 디렉터리는 반복되는 묶음이나 함께 유지해야 할 local context를 실제로 더 잘 드러낼 때 만듭니다.
- 구조적으로 동등한 선택지라면 탐색하기 쉬운 쪽을 선호합니다.
- 작은 README나 index로 충분하면 불필요한 이동이나 계층 추가를 피합니다.
- 탐색성을 이유로 wrapper, 중복 abstraction, 불필요한 hierarchy를 만들지 않습니다.
...
현재처럼 source와 `tests/`를 분리한 pytest layout을 유지합니다. Repository가 직접 소유하는 executable source에 자연스러운 anchor가 있으면 관련 test도 그 위치를 예측할 수 있게 배치합니다.
...
- 같은 source의 behavior가 늘어나면 같은 디렉터리의 sibling test files로 나눌 수 있습니다.
- 반복되는 prefix, fixture·data·snapshot 같은 local context, 높은 파일 밀도로 묶음의 이점이 커지면 bundle directory를 고려합니다.
...
Source를 이동하거나 이름을 바꿀 때는 관련 test의 탐색 관계도 함께 확인합니다. 다만 더 안정적인 behavior boundary를 훼손하면서까지 literal mirroring하지 않습니다.
