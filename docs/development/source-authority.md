...
| 결정 대상 | 권한을 가진 곳 |
| --- | --- |
| 사람이 수정할 원본 형식 | 선택한 소스 프레임워크 또는 벤더 고유 형식 |
...
구체적인 내부 구조와 스키마는 저장소에서 별도 확장 규격을 만들지 않고, 선택한 소스 프레임워크 또는 벤더의 현재 공식 계약을 따릅니다.
...
이 문서는 Rulesync 스키마, Agent Skills 명세, 벤더 동작 또는 개별 Skill 작업 흐름을 다시 설명하지 않습니다. 현재 운영 규칙, 도구 통합, 소스 등록 정보와 개별 자산의 작업 흐름은 각각의 더 좁은 소유자가 관리합니다.
