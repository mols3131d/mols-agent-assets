...
| 원칙 | 핵심 |
| --- | --- |
| **효과와 정확성을 우선합니다.** | 우아함보다 실제 요구사항과 올바른 동작을 우선합니다. |
| **최소한의 충분한 복잡성을 선택합니다.** | KISS + YAGNI. 복잡성이 필요성을 증명해야 합니다. |
| **가장 작은 coherent change를 만듭니다.** | 줄 수가 아니라 conceptual surface를 최소화합니다. |
| **기존 시스템에 맞춥니다.** | 동등한 선택이면 새로운 pattern보다 기존의 좋은 pattern을 우선합니다. |
| **안정된 개념만 추상화합니다.** | 단순 duplication만으로 DRY abstraction을 만들지 않습니다. |
| **운영·유지보수 비용을 고려합니다.** | technical purity보다 이해·수정·진단·운영 용이성을 우선합니다. |
| **근거에 비례해 검증합니다.** | 추측보다 observable contract, 실제 failure와 evidence를 우선합니다. |
