...
5. 중복 검사에는 `rg "<term>" <skill-dir>`을, 컨텍스트 크기 검사에는 `wc -l <files>`와 같이 필요한 기능에 집중된 확인 방식을 선호합니다.
6. 요청된 문제를 해결하는 가장 작은 단위의 변경을 적용합니다. 기존 스킬의 전체 뼈대를 재구성하지 마십시오.
7. 인덱스나 레이아웃을 변경할 때는 `references/routing-skill-structure.md`를 읽으십시오. 선택, 모호성 또는 로딩 방식을 변경할 때는 `references/routing-skill-algorithm.md`를 읽으십시오.
8. `python3 scripts/validate_asset.py <skill-dir> --type skill`을 다시 실행합니다. 오류가 사라지고 경고가 수정되거나 명시적으로 수용될 때까지 요청되거나 실패한 영역만 수정합니다.
