...
- deployable `src/agentsmesh/skills/<skill-name>/` package 안에는 repository eval을 두지 않는다.
- deterministic assertion으로 판정 가능한 계약은 model grader보다 우선한다.
- runtime behavior를 검증하지 않은 case를 runtime evidence처럼 표현하지 않는다.

## Current suites

- `regression/agentsmesh-source-isolation.json` — `src/agentsmesh/` canonical authority와 repository runtime-discovery surface 격리를 고정하는 deterministic contract.
