...
- 이 repository 자체를 작업하는 agent의 routing entrypoint는 `.agents/route/ROUTE.md`입니다. Family membership은 `.agents/route/families.json`이 소유하고, `.agents/route/routes.jsonl`과 나머지 `*.jsonl`은 lock/config와 family membership에서 생성되는 discovery data이므로 직접 편집하지 않습니다.
