...
- `skills-lock.json`의 외부 Skill dependency가 필요하거나 설치 상태의 최신성이 불명확하면 installer를 직접 조합하지 말고 `mise run skills-sync`를 사용합니다. 이 task는 lock의 source/revision을 읽되 lock을 수정하지 않으며, vendor별 payload·설치 방식은 해당 dependency의 native installer에 위임합니다.
