---
name: mols-agent-asset-studio-legacy
description: >-
...
1. Identify the requested outcome, operation, object, and constraints.
1. Compare the request with each workflow `description`.
1. Select the minimum workflow set covering the request.
1. Resolve material ambiguity with one targeted question.
1. Resolve each selected `name` as `workflows/<name>.md`.
1. Load referenced resources only when a selected workflow requires them.
1. Run each selected workflow's validation before completion.
