...
1. `avoid_when`과 일치하는 route를 제거한다.
1. `use_when`을 기준으로 요청을 충족하는 최소 route set을 선택한다.
1. 선택된 exact `entrypoint`만 읽는다.
1. Workflow가 요구하는 reference와 script만 추가로 사용한다.
1. 선택된 모든 Workflow의 validation을 통과한 뒤 완료한다.
...
1. `rg`, `wc`, scaffold, validator 같은 결정적 command를 먼저 사용한다.
1. Skill의 job, trigger, output, near-miss exclusion을 정의한다.
1. 필요한 최소 구조만 사용한다.
1. 공통 workflow는 `SKILL.md`, 상세 지식은 `references/`, 반복 로직은 `scripts/`, 출력 재료는 `assets/`에 둔다.
1. 검증과 수정을 반복한다.
...
1. 기존 자산 rename은 사용자가 명시적으로 요청했는지 확인한다.
1. `references/naming-convention.md`를 읽는다.
1. Domain과 job을 식별하는 가장 짧은 이름을 선택한다.
1. Rename을 적용했다면 내부 reference를 갱신한다.
