---
name: routing-skill-structure
description: >
  USE WHEN: establishing the folder hierarchy, workflow indices, or composite skill entry points for a routing skill architecture.
  EXCLUDES: dedicated router asset validation, CLI command flag conventions, or visualization creation.
---
...
├── workflows/        # Routed workflow groups
│   ├── INDEX.csv     # Root route registry
│   └── <group>/INDEX.csv
...
- `INDEX.csv`: **Route Index**; each index routes only assets in its own directory.
- routed Markdown files: **Workflow Modules** (including Command Flag Workflows like `workflows/--config.md`, `workflows/--init.md`, `workflows/--help.md`).

See [references/command-flag-workflows.md](command-flag-workflows.md) for flag workflow naming, registration, and routing guidelines.
See [references/routing-skill-visualizations.md](routing-skill-visualizations.md) for Mermaid architectural diagrams of skill and router patterns.
...
A routing skill uses a root `workflows/INDEX.csv` plus optional child indexes. Each index has fields `id,use_when,excludes`, and each `id` resolves from the directory containing that index. Child indexes reduce the amount of route metadata loaded per task.
