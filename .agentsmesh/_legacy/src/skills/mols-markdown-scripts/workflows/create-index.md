...
2. Run `scripts/generate_index.py` with the target directory.
3. Select one format:
...
4. 대상 관행이나 사용자 요청에 맞는 출력 경로를 `--output`으로 지정한다.
   출력 파일명이 지정되지 않았으면 stdout으로 결과를 반환한다.
5. 대상 관행이 없고 파일명이 필요할 때만 `INDEX.csv`를 기본 권장값으로
   사용한다. 이는 고정 규칙이 아니다.
6. Use `--require-fields` when missing frontmatter fields must fail generation.
7. Use `--unique-fields` when duplicated frontmatter values must fail generation.
8. Review the generated file and run the Markdown formatter if needed.
