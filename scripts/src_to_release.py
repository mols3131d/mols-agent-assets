...
def _should_ignore(name: str) -> bool:
    suffixes = [ext.lstrip(".") for ext in Path(name).suffixes]
    return any(ext in IGNORE_EXTENSIONS for ext in suffixes)


def ignore_files(dir_path: Any, contents: list[str]) -> list[str]:
    return [name for name in contents if _should_ignore(name)]
...
def copy_to_staging(source: Path, staging: Path) -> None:
    ...
    if source.is_dir():
        ...
        if _should_ignore(source.name):
            raise ValueError(
                f"무시되는 확장자가 포함된 파일은 승격할 수 없습니다: {source.name}"
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=f"archive 디렉터리 경로 (기본값: {DEFAULT_ARCHIVE_DIR})",
    )
    return parser
