...
- PR conversation comment의 optional workflow status는 `🔴 Blocked`, `🟡 Waiting`, `🟢 Unblocked`를 사용합니다. `Blocked`는 다음 작업을 진행할 수 없는 상태, `Waiting`은 외부 결과나 의존성을 기다리는 상태, `Unblocked`는 명시한 다음 작업을 막는 알려진 blocker가 없는 상태에만 사용합니다. GitHub의 `Ready for review` 같은 native lifecycle 상태를 대신 표현하지 않습니다.
