...
4. **P2 — Absolute presentation rules are too broad.** Higher-authority/runtime-required content and explicit artifact style must win.
5. **P2 — Intensity control is useful but under-specified.** Keep one public `intensity` control with `default | auto | lite | full | ultra`; do not add a larger configuration surface.
6. **P2 — Experimental lifecycle belongs in maintainer docs.** Keep the runtime package focused and move WIP/maturity information out.
7. **P2 — No eval surface exists.** Add compact trigger and behavior cases, especially near-miss negatives and semantic-preservation cases.
...
- Do not add supporting runtime files unless the behavior cannot remain clear in one `SKILL.md`.
- Preserve `agentsskills.license: MIT`.
