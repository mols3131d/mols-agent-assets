...
이 질문에 영향을 주지 않는 cleanup, generic best-practice review 또는 unrelated exploration은 하지 않는다.
...
- reference는 판단 보조 자료이며 존재 자체를 correctness evidence로 취급하지 않는다.
- supplied path/identifier가 있으면 필요한 부분을 읽되, target/source/test/configuration/state와 관련 문서·자산은 candidate를 검증하는 데 필요한 만큼 스스로 탐색한다.
