...
- **Declared module contract** — module 간 선언된 dependency 구조를 보려면 `tach show`를 우선한다. Exact output semantics나 option이 판단에 중요하면 현재 Tach version의 authoritative source를 확인한다.
...
이 세 surface를 같은 의미로 취급하지 않는다. Contract graph와 observed dependency evidence가 다르면 `SKILL.md`의 Core Contract에 따라 mismatch를 판단한다.
...
- Graph가 **declared contract**인지 **observed dependency**인지 결과에 명시한다. 둘을 결합하면 edge나 layer를 구분 가능한 방식으로 표현한다.
