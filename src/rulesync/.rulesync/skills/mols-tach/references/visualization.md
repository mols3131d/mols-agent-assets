...
- **Declared dependency edges** — module의 선언된 dependency 관계를 보려면 `tach show`를 우선한다. Local output은 현재 Tach에서 Graphviz DOT 또는 Mermaid로 만들 수 있다. Exact output semantics나 option이 판단에 중요하면 현재 Tach version의 authoritative source를 확인한다.
- **Observed file dependencies** — 실제 file-level dependency를 machine-readable data나 custom analysis에 쓰려면 `tach map`을 고려한다.
...
- Remote viewer는 local output의 기본 대체재로 가정하지 않는다. 외부 전송, 현재 지원 상태와 repository policy가 material하면 사용 전에 확인한다.
...
- `tach show`를 Tach architecture contract 전체의 완전한 시각화로 해석하지 않는다. 현재 local graph는 선언된 module dependency edge를 중심으로 하며 layer, interface, visibility 같은 별도 contract semantics를 모두 표현하지 않는다.
...
- Runtime/dynamic coupling처럼 Tach가 관찰하지 못하는 관계는 시각화에도 나타났다고 주장하지 않는다.
