...
## Basic: Explicit Participants And Messages
...
Participant와 message order는 source-backed interaction을 보존한다. Code-like appearance가 runtime implementation language, transport protocol 또는 blocking behavior를 자동으로 증명하지 않는다.

## Message Kind Is Semantic

ZenUML은 sync, async, creation, reply message를 서로 다른 interaction으로 표현할 수 있다.

- Sync message는 blocking call 의미가 source에 있을 때만 사용한다.
- Async message는 fire-and-forget/non-blocking interaction이 실제로 확인될 때만 사용한다.
...
1. Sync/async/creation/reply kind가 source보다 강한 runtime semantics를 만들지 않는가.
...
1. `sequenceDiagram` fallback이 condition, order, participant와 message kind의 중요한 차이를 보존하는가.
