...
- node, edge와 boundary는 독립적인 semantic information을 가져야 한다. 반복 unit의 **identity, responsibility, ownership, state, provenance, boundary와 relationship이 모두 별도 의미를 갖지 않을 때만** 병합을 검토한다.
