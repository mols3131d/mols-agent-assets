...
DECLARED_PATH_PATTERN = re.compile(
    r"`((?:agents|subagents|prompts|instructions|tools|guardrails|references|templates|"
    r"configs?|schemas|scripts|evals|docs)/[^`\s]+)`"
)
HEADING_PATTERN = re.compile(r"^(#{1,6})\s+\S")
NORMATIVE_PATTERN = re.compile(
    r"\b(?:must|should|never|always|required|forbidden)\b|"
    r"(?:반드시|항상|절대|금지|해야 한다|해야한다|필수)",
    re.IGNORECASE,
)
...
def iter_entries(root: Path) -> Iterable[Path]:
    """Yield scan-worthy files and symlink directories without traversing symlinks."""
...
def read_text(path: Path, findings: list[Finding], root: Path) -> str | None:
    ...
    try:
        ...
        findings.append(
            Finding("major", "encoding", rel, "text-like file is not valid UTF-8")
        )
        return None
...
def normalized_normative_lines(text: str) -> list[str]:
    ...
    for raw in text.splitlines():
        ...
        if in_fence or len(stripped) < 20 or not NORMATIVE_PATTERN.search(stripped):
            continue
        normalized = re.sub(
            r"\s+", " ", stripped.lstrip("-*0123456789. ")
        ).casefold()
        if normalized:
...
def relation_target(path: Path, raw_target: str, root: Path) -> str | None:
    ...
    if not target or target.startswith(
        ("#", "http://", "https://", "mailto:", "sandbox:", "data:")
    ):
        return None
...
def check_declared_paths(
    path: Path, text: str, root: Path, findings: list[Finding]
) -> None:
    rel = relative(path, root)
        ...
        if not safe_candidates:
            findings.append(
                Finding(
                    ...
                    "reference",
                    rel,
                    f"declared path escapes target root: {raw_target}",
                )
            )
            continue
        if not any(candidate.exists() for candidate in safe_candidates):
            findings.append(
                Finding(
                    ...
                    "reference",
                    rel,
                    f"declared asset path does not exist: {raw_target}",
                )
            )
        ...
        try:
            ...
            findings.append(
                Finding(
                    ...
                    "link",
                    rel,
                    f"relative link escapes target root: {raw_target}",
                )
            )
            continue
        if not resolved.exists():
            findings.append(
                Finding("major", "link", rel, f"broken relative link: {raw_target}")
            )
...
def check_text_file(
    ...
    for number, line in enumerate(text.splitlines(), start=1):
        if line.rstrip() != line:
            findings.append(
                Finding("minor", "format", rel, f"trailing whitespace at line {number}")
            )
    if path.suffix == ".md":
        if text.count("```") % 2:
            findings.append(
                Finding("major", "markdown", rel, "unbalanced fenced code block")
            )
        check_markdown_links(path, text, root, findings)
        check_declared_paths(path, text, root, findings)
        for number, previous, current in heading_depth_jumps(text):
            findings.append(
                Finding(
                    "minor",
                    "human-comprehension",
                    rel,
                    f"heading depth jumps from H{previous} to H{current} at line {number}",
                )
            )
        normative = normalized_normative_lines(text)
        metadata["normative_lines"] = len(normative)
        metadata["normative_density"] = round(
            len(normative) / max(1, len(text.splitlines())), 4
        )
        if len(text.splitlines()) > 500 and asset_type in {
            "skill",
            "agent",
            "subagent",
            "instruction",
            ...
            findings.append(
                Finding(
                    ...
                    "context-cost",
                    rel,
                    "agent-facing text exceeds 500 lines; review progressive disclosure and relevance",
                )
            )
        if len(text.splitlines()) > 100 and metadata["normative_density"] > 0.35:
            findings.append(
                Finding(
                    ...
                    rel,
                    "high normative-line density is a heuristic signal requiring semantic review",
        ...
        if identity_asset and not frontmatter:
            findings.append(
                Finding("major", "frontmatter", rel, "agent asset is missing frontmatter")
            )
        if frontmatter:
            ...
            if identity_asset:
                ...
                if (
                    asset_type == "skill"
                    and frontmatter.get("name")
                    and not NAME_PATTERN.fullmatch(frontmatter["name"])
                ):
                    findings.append(
                        Finding(
                            "major",
                            "identity",
                            rel,
                            "Skill frontmatter name must be kebab-case",
                        )
                    )
    for name, pattern in SECRET_PATTERNS.items():
        if pattern.search(text):
            findings.append(
                Finding("critical", "secret", rel, f"possible embedded {name}")
            )
    return metadata
...
def check_python(path: Path, root: Path, findings: list[Finding]) -> None:
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            py_compile.compile(
                path, cfile=str(Path(temp_dir) / "compiled.pyc"), doraise=True
            )
    except py_compile.PyCompileError as exc:
        findings.append(
            Finding("major", "python", rel, f"Python compile failure: {exc.msg}")
        )
...
def scan_directory(root: Path) -> dict[str, object]:
        ...
        if path.is_symlink():
            findings.append(
                Finding(
                    ...
                    "package",
                    rel,
                    "symlink found; review target and archive semantics",
                )
            )
            continue
        asset_type = classify(path, root)
        ...
        if path.stat().st_size > 1024 * 1024:
            findings.append(
                Finding(
                    ...
                    "size",
                    rel,
                    "file is larger than 1 MiB and may increase context cost",
                )
            )
            ...
            if text is not None:
                ...
                if path.suffix.lower() == ".md":
                    for raw_target in MARKDOWN_LINK_PATTERN.findall(text):
                        ...
                        if target:
                            relationships.add((rel, "reads", target))
                    for raw_target in DECLARED_PATH_PATTERN.findall(text):
                        target = relation_target(path, raw_target, root)
                        if target:
                            relationships.add((rel, "reads", target))
                ...
                if (
                    asset_type in IDENTITY_ASSET_TYPES
        ...
        if len(paths) > 1:
            findings.append(
                Finding(
                    ...
                    "identity",
                    ", ".join(paths),
                    f"duplicate frontmatter name: {name}",
                )
            )
        ...
        if len(paths) >= 3:
            ...
            findings.append(
                Finding(
                    ...
                    "context-noise",
                    rendered_paths,
                    f"identical normative instruction is repeated across {len(paths)} files: {normalized[:120]}",
                )
            )
    ...
    skill_paths = [
        ...
        if item["path"] == "SKILL.md" or str(item["path"]).endswith("/SKILL.md")
    ]
    if len(skill_paths) > 1:
        findings.append(
            Finding(
                ...
                "package",
                ".",
                "multiple SKILL.md files found; validate package roots explicitly",
            )
        )
    ...
    longest = max(
        text_items, key=lambda item: int(item.get("lines", 0)), default=None
    )

    rendered_relationships = [
        {"from": source, "type": relation_type, "to": target}
        for source, relation_type, target in sorted(relationships)
    ]
    return {
        ...
        "analysis_signals": {
            ...
            "relationship_count": len(rendered_relationships),
            "longest_text_file": None
            if longest is None
            else {"path": longest["path"], "lines": longest["lines"]},
        },
...
def main() -> int:
    ...
    if args.strict and sum(
        int(summary[key]) for key in ("critical", "major", "minor", "note")
    ):
        return 1
