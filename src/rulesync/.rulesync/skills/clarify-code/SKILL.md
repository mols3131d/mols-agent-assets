...
주된 surface는 caller-facing API documentation, code-local comment와 source-level module/package explanation이다. 코드 구조 자체가 이해 비용의 원인이면 prose로 덮지 않고 `code-comprehension-refactor`를 사용한다.
