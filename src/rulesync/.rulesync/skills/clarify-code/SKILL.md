...
Evidence-backed durable meaning이 남아 있는데 comment를 피하기 위해 no-op으로 조기 종료하지 않는다. 반대로 semantic claim을 뒷받침할 evidence가 없거나 필요한 evidence가 허용된 scope 밖에 있거나 추가 prose가 제거하는 이해 비용보다 읽기·유지 비용이 크면 설명을 invent하지 않고 중단할 수 있다.
