...
주된 surface는 caller-facing API documentation, code-local comment와 module/package-level explanation이다. 코드 구조 자체가 이해 비용의 원인이면 prose로 덮지 않고 `code-comprehension-refactor`를 사용한다.
...
| 한 symbol보다 file/package 전체에 안정적으로 적용되는 local convention | module/package-level documentation surface |
...
Evidence-backed durable meaning이 남아 있는데 explanation을 피하기 위해 no-op으로 조기 종료하지 않는다. 반대로 semantic claim을 뒷받침할 evidence가 없거나 conflict가 해소되지 않았거나 필요한 evidence가 허용된 scope 밖에 있거나 추가 prose가 제거하는 이해 비용보다 읽기·유지 비용이 크면 explanation을 invent하지 않고 중단할 수 있다.
...
- module/package-level explanation이 적절한지 판단해야 함
