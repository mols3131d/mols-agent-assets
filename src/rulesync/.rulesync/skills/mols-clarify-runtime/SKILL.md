...
description: >-
  ...
  the smallest existing executable scenario and native runtime evidence; change
  persistent evidence only when current observation is insufficient or the same
  question should be answerable later without retracing the code. Preserve
  observable behavior and existing caller or consumer contracts. Do not use for
  defect debugging, correctness review, test design, monitoring or
...
기능이나 domain behavior를 추가하지 않고 runtime 이해 부채를 줄인다. **실행 후 답하기 어려운 가장 중요한 복원 질문 하나를 고르고, 먼저 기존 실행과 evidence를 관찰한다. 그래도 부족할 때만 가장 작은 persistent evidence 변경을 선택한다.**
...
| `--evidence <value\|auto>` | 기존 observation surface를 먼저 사용하고, 필요하면 가장 작은 persistent evidence 변경을 선택한다. |
...
1. 기존 observation만으로 질문에 답할 수 있으면 수정하지 않는다. 부족하면 공백의 형태를 진단하고, 이번 조사에만 필요한 transient observation인지 미래에도 반복해서 답해야 할 persistent evidence인지 구분한다.
1. Persistent evidence가 필요하면 가장 작은 변경 방식과 owner를 고른다. 기존 evidence를 재사용·보강·이동·병합·제거하는 것을 새 evidence 추가보다 먼저 고려하고, semantic owner뿐 아니라 실제 생성·가용성·보존·접근성과 필요한 correlation도 확인한다. [Evidence](references/evidence.md)와 필요한 경우 [Correlation](references/correlation.md)을 따른다.
...
| result, exception, artifact/metadata, history, log 등 persistent evidence owner와 viability 선택 | [Evidence](references/evidence.md) |
...
- logging이나 persistent evidence 추가를 기본 해법으로 가정하지 않는다. 기존 실행 관찰이나 정리·이동·병합·제거가 더 작은 해법이면 그것을 사용한다.
