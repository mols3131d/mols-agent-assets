...
Runtime evidence는 목적이 아니라 **복원 질문에 답하기 위한 surface**다. 먼저 기존 execution을 관찰하고, 미래에도 같은 질문을 반복해서 답해야 하거나 현재 evidence가 실제 reconstruction에 부족할 때만 persistent evidence를 바꾼다.

## Observation And Persistent Evidence
...
| Persistent evidence | 나중에도 code를 다시 추적하지 않고 같은 질문에 답한다 | result/exception context, durable metadata/artifact, existing trace/span data, log/event, correlation field |

Observation만으로 질문이 풀리면 persistent code/evidence를 추가하지 않는다.

## Choose The Owner

Persistent evidence가 필요하면 가장 자연스러운 기존 owner를 고른다.
...
Persistent evidence를 선택하기 전에 필요한 범위에서 확인한다.
...
모든 evidence가 영구적일 필요는 없다. 필요한 사용 시점보다 충분히 오래 남으면 된다. 반대로 sampling되거나 쉽게 drop되는 telemetry를 **반드시 복원되어야 하는 사실의 유일한 owner**로 두지 않는다.

## Selection Rules

1. **누가 이 사실을 알아야 하는가?** caller, maintainer, operator, downstream Agent 중 실제 독자를 정한다.
1. **기존 observation으로 충분한가?** 충분하면 persistent evidence를 추가하지 않는다.
1. **기존 durable owner가 있는가?** result, exception, artifact, framework metadata/history가 충분하면 wrapper log를 추가하지 않는다.
...
| --- | --- |
| existing test/result만으로 선택한 behavior를 충분히 이해할 수 있다 | 코드 변경 없이 stop한다. |
...
- field/event/links 제한 때문에 정작 중요한 evidence가 truncate/drop될 가능성이 커진다.
