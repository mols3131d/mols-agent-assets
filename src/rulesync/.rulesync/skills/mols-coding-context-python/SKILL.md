...
- Import/package 문제를 `sys.path` 수정, `PYTHONPATH` 주입 또는 working-directory 변경으로 먼저 우회하지 않는다. Package layout, entrypoint와 실행 방식을 확인하고 ambient cwd에 우연히 의존하는 import를 만들지 않는다.
- Async/concurrent code에서는 blocking work, task owner와 lifetime, cancellation, failure propagation과 event-loop ownership을 실제 lifecycle에 맞춘다. Cancellation을 ordinary error로 삼키지 않으며, 상위 caller/runtime이 event-loop lifecycle을 소유하는 context에서는 `asyncio.run()`이나 새 loop 생성으로 별도 lifecycle을 만들지 않는다. `async`나 concurrency를 일반적인 개선책으로 도입하지 않는다.
- Shared mutable state의 correctness를 GIL이나 문서화되지 않은 built-in operation의 atomicity에 의존시키지 않는다. 필요한 synchronization과 ownership은 target interpreter/build의 documented guarantees와 실제 concurrency model에 맞춘다.
