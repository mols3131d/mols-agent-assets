...
- Async/concurrent code에서는 blocking work, task owner와 lifetime, cancellation, failure propagation과 event-loop ownership을 실제 lifecycle에 맞춘다. Cancellation을 ordinary error로 삼키거나 caller가 event-loop lifecycle을 소유하는 context에서 `asyncio.run()` 또는 새 loop 생성으로 그 ownership을 탈취하지 않으며, `async`나 concurrency를 일반적인 개선책으로 도입하지 않는다.
