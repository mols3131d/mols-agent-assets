...
- Async/concurrent code에서는 blocking work, task owner와 lifetime, cancellation, failure propagation과 event-loop ownership을 실제 lifecycle에 맞춘다. Cancellation을 ordinary error로 삼키지 않으며, 상위 caller/runtime이 event-loop lifecycle을 소유하면 `asyncio.run()`이나 새 loop 생성으로 그 경계를 우회하지 않는다. `async`나 concurrency를 일반적인 개선책으로 도입하지 않는다.
