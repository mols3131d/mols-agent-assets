...
해법은 code smell 이름에 기계적으로 매핑하지 않는다. **현재 coherent bottleneck을 실질적으로 줄이면서 observable/usage contract와 material performance를 보존하는 가장 작은 안전한 change**를 고른다.

Core `SKILL.md`가 common-path 선택을 소유한다. 이 reference는 transformation family의 preservation risk, tightly coupled edit 또는 net comprehension trade-off가 단순하지 않을 때 사용한다.

## Before Transforming

Transformation을 고르기 전에 세 가지를 확인한다.

1. 어떤 reader work를 제거하려는가.
2. 어떤 observable/usage surface를 건드릴 수 있는가.
3. 현재 language, repository 또는 refactoring tooling에서 이 transformation에 material한 precondition이 무엇인가.

Portable Skill에 language별 precondition catalog를 복제하지 않는다. Rename, move, extract, inline, representation/control-state rewrite가 선택되면 적용되는 language/runtime/tooling contract에서 relevant risk만 확인한다.

Native refactoring tool이나 usage preview가 실제 repository에서 사용할 수 있다면 좋은 evidence가 될 수 있다. 그러나 static reference 결과, dynamic-usage heuristic 또는 tool success 자체를 behavior preservation proof로 취급하지 않는다.

## Intervention Map

| Cost source | Prefer | Avoid |
| --- | --- | --- |
| lexical/domain decoding | repository-established domain term, specific internal name/API | 단순히 더 긴 이름, 새 local synonym, unverified dynamic usage가 있는 rename |
...
| compound/negative flow | guard clause, positive condition, named condition | exit를 늘려 전체 흐름을 더 분산하거나 evaluation/order semantics 변경 |
| state/order reasoning | state owner, phase, ordering을 code structure에 더 직접 반영 | 순서 contract를 comment에만 의존하거나 side-effect order를 검증 없이 변경 |
...
## Rename by Meaning and Usage

Rename은 lexical decoding cost를 크게 줄일 수 있지만 이름이 observable surface일 수 있다.

다음 순서로 본다.

1. 현재 이름이 실제로 domain/role decoding을 요구하는가.
2. repository에 이미 established term이 있는가.
3. rename 대상이 source reference 외에 framework/config/reflection/string/generated usage에 연결될 가능성이 current evidence로 material한가.
4. known usages를 안전하게 함께 변경할 수 있는가.

`tmp`를 무조건 긴 이름으로 바꾸는 것이 목표가 아니다. Scope 안에서 충분히 local하고 role이 이미 명확한 이름은 그대로 둘 수 있다. 반대로 established domain term을 도입하면 여러 reader의 반복 translation을 줄이는 경우 rename이 작은 고가치 intervention일 수 있다.
...
Representation이 persisted/wire/schema shape이거나 external consumer가 직접 읽는 형태라면 local implementation detail로 가정하지 않는다.
...
중간 이름이 한 번 더 해석해야 하는 alias일 뿐이면 만들지 않는다. Condition extraction이 evaluation order, short-circuiting, exception timing 또는 side effect를 바꾸지 않는지도 relevant할 때 확인한다.
...
Inline/move/extract는 execution count, state capture, side-effect ordering, name binding 또는 identity를 바꿀 수 있다. 그런 risk가 실제 transformation에 material하면 적용되는 language/tool/repository contract에서 precondition을 확인한다.
...
- high-risk rename/move/representation/control-state change인데 relevant usage 또는 preservation evidence가 부족하다. 더 작은 intervention을 고르거나 중단한다.
