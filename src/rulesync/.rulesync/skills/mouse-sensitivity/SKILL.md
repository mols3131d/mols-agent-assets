...
- 게임의 native sensitivity는 해당 게임과 입력 상태 안에서 해석한다. 다른 게임의 숫자를 직접 비교하거나 복사하지 않는다.
- 결론을 바꾸는 정보만 요구한다. DPI 외의 마우스 모델, polling rate, sensor, 무게 같은 hardware metadata는 필요가 입증되지 않으면 묻지 않는다.
- 정확한 게임 간 환산은 기본 동작이 아니다. 사용자가 요청하거나 결론에 필요할 때만 현재 외부 자료를 조사한다.

## Workflow

1. 현재 목적과 input context, 실제 적용되는 설정을 확인한다.
2. 필요한 reference만 읽고 해당 입력에 영향이 없는 설정을 제외한다.
3. 현재 값 주변의 작은 조정부터 제안한다. 정확한 변환이 필요할 때만 현재 자료를 조사한다.
4. 같은 작업이나 플레이 상황에서 결과를 확인한다. 사용자가 요청하면 확인된 값만 profile에 반영한다.
