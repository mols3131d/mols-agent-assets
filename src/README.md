...
The peer Agent Asset types used by this repository are **Rule, Skill, Prompt, and Agent**. References, docs, scripts, assets, evals, and tests are supporting resources rather than peer asset types.
...
Rule deployment also uses a **repository-local, non-standard convention**: root/nested `AGENTS.md` for directory scope and target-appropriate glob selectors for common subdirectories, file groups, or extensions. See `rules/README.md` and `../docs/references/agent-asset-boundaries.md`.
