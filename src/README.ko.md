...
| 디렉터리 | 현재 역할 |
| --- | --- |
| `rules/` | 현재 AgentsMesh contract 밖의 custom 또는 target-specific Rule |

`src/skills/`, `src/skills-chatbot/`, `src/skills-chatbot-runtime/`, `src/agents/`, `src/prompts/`를 parallel source로 다시 만들지 않습니다. 일반 `<name>/SKILL.md` package로 표현할 수 있는 Skill은 single-file이든 bundled이든 `.agentsmesh/skills/`에 둡니다.
