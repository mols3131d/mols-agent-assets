...
def analyze_diff(diff_text):
    ...
    sec_keywords = [
        (re.compile(r"\b(password|passwd|secret|token|api[-_]?key|private[-_]?key|credential)\b", re.IGNORECASE), "Potential credential/secret leak"),
        (re.compile(r"\b(eval|exec|system|popen)\b"), "Unsafe command execution function"),
        (re.compile(r"\b(unsafe|innerHTML|dangerouslySetInnerHTML)\b"), "Potential unsafe execution or XSS risk")
    ]
    
    perf_keywords = [
        (re.compile(r"\b(select\s+\*|insert\s+into|update\s+|delete\s+from)\b", re.IGNORECASE), "SQL Query found (verify index and N+1 query safety)"),
        (re.compile(r"\b(while\s*\(true\)|for\s+.*\b(in|of)\b.*inside\s+loop|nested\s+loops)\b", re.IGNORECASE), "Potential loop efficiency check needed")
    ]
